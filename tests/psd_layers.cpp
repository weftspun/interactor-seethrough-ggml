...
unsigned int find_channel(Layer *layer, int16_t type) {
	for (unsigned int i = 0; i < layer->channelCount; i++) {
		if (layer->channels[i].data && layer->channels[i].type == type) {
			return i;
		}
	}
	return UINT_MAX;
...
int main(int argc, char **argv) {
	if (argc < 3) {
		fprintf(stderr, "usage: %s <file.psd> <out_dir>\n", argv[0]);
		return 1;
	}
	...
	if (!document) {
		fprintf(stderr, "cannot parse PSD document\n");
		file.Close();
		return 1;
	}
	...
	if (!lms) {
		fprintf(stderr, "no layer/mask section\n");
		DestroyDocument(document, &allocator);
		file.Close();
		return 1;
	}
	...
	for (unsigned int i = 0; i < lms->layerCount; i++) {
		...
		Image img;
		img.w = document->width;
		img.h = document->height;
		img.c = 4;
