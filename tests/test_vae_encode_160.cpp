...
int main(int argc, char **argv) {
	if (argc < 3) {
		fprintf(stderr, "usage: %s vae.gguf reference.bin\n", argv[0]);
		return 1;
	}
	...
	if (getenv("SEETHROUGH_TILED_ATTN")) {
		m.tiled_naive_attn = true;
	}
	...
	for (auto &dt : m.debug_taps) {
		...
		for (float f : v) {
			sum += f;
			sum2 += (double)f * f;
			amax = std::max(amax, (double)fabs(f));
		}
