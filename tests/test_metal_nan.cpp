...
int main(int argc, char ** argv) {
    if (argc < 2) { fprintf(stderr, "usage: %s layerdiff-unet.gguf\n", argv[0]); return 1; }
    ...
    Model m;
    if (!m.load_backend(argv[1], ggml_backend_dev_buffer_type(d))) { fprintf(stderr, "load failed\n"); return 1; }
    ...
    for (auto * t : { sample, ehs, text, tids, ts }) ggml_set_input(t);
    ...
    for (auto & v : samp) v = (float)rand() / RAND_MAX * 2 - 1;
    ...
    for (int f = 0; f < F; f++) { float ids[6] = {(float)RES,(float)RES,0,0,(float)RES,(float)RES}; for (int i=0;i<6;i++) tid_v[f*6+i]=ids[i]; }
    ...
    for (size_t i = 0; i < result.size(); i++) {
        ...
        else if (std::isinf(result[i])) inf_count++;
        else max_abs = std::max(max_abs, std::abs((double)result[i]));
    }
