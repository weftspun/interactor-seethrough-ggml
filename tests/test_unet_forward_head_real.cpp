...
int main(int argc, char **argv) {
	if (argc < 3) {
		fprintf(stderr, "usage: %s layerdiff-unet.gguf reference.bin\n", argv[0]);
		return 1;
	}
	...
	for (ggml_tensor *t : { sample, ehs, text, tids, ts }) {
		ggml_set_input(t);
	}
	...
	for (size_t i = 0; i < taps.size() && i + 5 < ref.size(); i++) {
		printf("-- %s --\n", tap_names[i]);
		if (compare_ref(taps[i], ref[5 + i]) != 0) {
			rc = 1;
		}
	}
	printf("-- final --\n");
	if (compare_ref(out, ref[4]) != 0) {
		rc = 1;
	}
	return rc;
}
