...
int main(int argc, char **argv) {
	if (argc < 3) {
		fprintf(stderr, "usage: %s layerdiff-vae.gguf trans-vae.gguf\n", argv[0]);
		return 1;
	}
	...
	if (!m.load_backend(argv[1], ggml_backend_dev_buffer_type(d))) {
		fprintf(stderr, "load vae failed\n");
		return 1;
	}
	if (!m.load_backend(argv[2], ggml_backend_dev_buffer_type(d))) {
		fprintf(stderr, "load trans-vae failed\n");
		return 1;
	}
