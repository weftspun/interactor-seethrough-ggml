...
ggml_tensor *vae_encode(Model &m, ggml_tensor *x, std::vector<ggml_tensor *> *taps) {
	...
	return ggml_cont(ctx, ggml_view_4d(ctx, sample, sample->ne[0], sample->ne[1], 4, sample->ne[3], sample->nb[1], sample->nb[2], sample->nb[3], 0));
}
...
static ggml_tensor *blend_edge(Model &m, ggml_tensor *a, ggml_tensor *b, int64_t extent, int dim) {
	...
	auto slab = [&](ggml_tensor *t, int64_t start, int64_t len) {
		...
		return ggml_cont(ctx, ggml_view_4d(ctx, t, ne0, ne1, t->ne[2], t->ne[3], t->nb[1], t->nb[2], t->nb[3], off));
	};
...
ggml_tensor *vae_encode_tiled(Model &m, ggml_tensor *x) {
	...
	if (H <= TILE_PX && W <= TILE_PX) {
		return vae_encode(m, x);
	}

	std::vector<int64_t> row0, col0;
	for (int64_t i = 0; i < H; i += OVERLAP_PX) {
		row0.push_back(i);
	}
	for (int64_t j = 0; j < W; j += OVERLAP_PX) {
		col0.push_back(j);
	}
	...
	for (size_t ti = 0; ti < row0.size(); ti++) {
		for (size_t tj = 0; tj < col0.size(); tj++) {
			...
			ggml_tensor *crop = ggml_cont(ctx, ggml_view_4d(ctx, x, tw, th, x->ne[2], x->ne[3], x->nb[1], x->nb[2], x->nb[3], j * x->nb[0] + i * x->nb[1]));
	...
	for (size_t ti = 0; ti < tiles.size(); ti++) {
		...
		for (size_t tj = 0; tj < tiles[ti].size(); tj++) {
			...
			if (ti > 0) {
				t = blend_edge(m, tiles[ti - 1][tj], t, BLEND_LAT, 1);
			}
			if (tj > 0) {
				t = blend_edge(m, tiles[ti][tj - 1], t, BLEND_LAT, 0);
			}
			const int64_t ch = std::min(t->ne[1], ROW_LIMIT), cw = std::min(t->ne[0], ROW_LIMIT);
			t = ggml_cont(ctx, ggml_view_4d(ctx, t, cw, ch, t->ne[2], t->ne[3], t->nb[1], t->nb[2], t->nb[3], 0));
...
ggml_tensor *unet1024(Model &m, ggml_tensor *x, ggml_tensor *latent) {
	...
	for (int i = 0; i < 7; i++) {
		...
		if (i == 3) {
			sample = ggml_add(ctx, sample, sample_latent);
		}
		for (int r = 0; r < 2; r++) {
	...
	for (int i = 0; i < 7; i++) {
		char pre[64], tapname[64];
		for (int r = 0; r < 3; r++) {
			ggml_tensor *res = res_stack.back();
			res_stack.pop_back();
...
ggml_tensor *trans_vae_decode(Model &m, ggml_tensor *z) {
	ggml_context *ctx = m.ctx_g;
	// Decode whole, never tiled. The SDXL VAE + unet1024 tiled paths matched
	// diffusers' tile/blend/crop yet still left a residual seam at the tile
	// boundary (page mid-line) that the linear blend didn't fully hide; the
	// reference PSD decodes untiled at these sizes. Removed entirely -- decode
	// fits VRAM at the pipeline's resolutions per-frame (NB=1).
