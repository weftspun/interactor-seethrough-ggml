...
static ggml_tensor *irfft2_mm(ggml_context *ctx, ggml_tensor *f, int64_t W,
	...
	ggml_tensor *Sr = ggml_cont(ctx, ggml_view_4d(ctx, f, Wh, H, C, N, f->nb[1], f->nb[2], f->nb[3], 0));
	ggml_tensor *Si = ggml_cont(ctx, ggml_view_4d(ctx, f, Wh, H, C, N, f->nb[1], f->nb[2], f->nb[3], C * f->nb[2]));
...
void lama_set_extra_inputs(const LamaExtraInputs &extra, int64_t img_W) {
	...
	for (const auto &p : extra.pad_idxs) {
		set_pad_idx(p);
	}
}
...
static void deinterleave_axis(ggml_tensor *t, int axis) {
	...
	if (t->buffer) {
		...
		std::memcpy(buf.data(), t->data, n * 4);
	}
	...
	if (t->buffer) {
		ggml_backend_tensor_set(t, out.data(), 0, n * 4);
	} else {
		std::memcpy(t->data, out.data(), n * 4);
	}
}
...
void lama_prepare_gpu_weights(Model &m) {
	for (int i = 5; i <= 22; i++) {
		for (const char *sub : { "conv1", "conv2" }) {
			...
			if (m.has(pre + ".conv_layer.bias")) {
				deinterleave_axis(m.get(pre + ".conv_layer.bias"), 0);
			}
			for (const char *p : { ".bn.weight", ".bn.bias", ".bn.running_mean", ".bn.running_var" }) {
				if (m.has(pre + p)) {
					deinterleave_axis(m.get(pre + p), 0);
				}
			}
		}
	}
...
static ggml_tensor *up_conv(Model &m, ggml_tensor *x, const std::string &pre) {
	...
	y = ggml_cont(ctx, ggml_view_4d(ctx, y, W2, H2, y->ne[2], y->ne[3], y->nb[1], y->nb[2], y->nb[3], y->nb[0] + y->nb[1]));
...
ggml_tensor *lama_inpaint_graph(Model &m, ggml_tensor *img, ggml_tensor *mask, LamaExtraInputs *extra) {
	...
	for (ggml_tensor *t : { fc.Ar, fc.Ai, fc.Br, fc.Bi, fc.Dr, fc.Di }) {
		ggml_set_input(t);
	}
	extra->fft_ar = fc.Ar;
	extra->fft_ai = fc.Ai;
