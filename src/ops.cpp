...
Model::~Model() {
	for (ggml_backend_buffer *b : bufs) {
		ggml_backend_buffer_free(b);
	}
	for (ggml_context *c : ctx_w) {
		ggml_free(c);
	}
}
...
ggml_tensor *conv2d(Model &m, ggml_tensor *x, const std::string &pre, int stride, int pad) {
	...
	if (m.conv_row_chunk && stride == 1 && pad == 1 &&
		...
		if (NCH == 1) {
			...
			if (m.has(pre + ".bias")) {
				r = ggml_add(ctx, r, bias4d(ctx, m.get(pre + ".bias")));
			}
			return r;
		}
		...
		for (int64_t y0 = 0; y0 < H; y0 += TS) {
			...
			if (y0 > 0 && y1 < H) {
				...
				ys = ggml_cont(ctx, ggml_view_4d(ctx, full, W, y1 - y0, w->ne[3], N, full->nb[1], full->nb[2], full->nb[3], (y0 - top) * full->nb[1]));
			}
			acc = acc ? ggml_concat(ctx, acc, ys, 1) : ys;
		}
		m.conv_row_chunk = rc;
		if (m.has(pre + ".bias")) {
			acc = ggml_add(ctx, acc, bias4d(ctx, m.get(pre + ".bias")));
		}
		return acc;
	}
...
ggml_tensor *linear(Model &m, ggml_tensor *x, const std::string &pre) {
	...
	if (flatten) {
		x = ggml_reshape_2d(ctx, x, x->ne[0], ne1 * ne2 * ne3);
	}
	...
	if (flatten) {
		y = ggml_reshape_4d(ctx, y, y->ne[0], ne1, ne2, ne3);
	}
	if (m.has(pre + ".bias")) {
		y = ggml_add(ctx, y, m.get(pre + ".bias"));
	}
	return y;
}
...
ggml_tensor *attn_block(Model &m, ggml_tensor *x, const std::string &pre) {
	...
	if (m.tiled_naive_attn) {
		...
		for (int64_t t0 = 0; t0 < T; t0 += chunk) {
			...
			ggml_tensor *qc = ggml_cont(ctx, ggml_view_4d(ctx, q, hd, t1 - t0, n_head, N, q->nb[1], q->nb[2], q->nb[3], t0 * q->nb[1]));
