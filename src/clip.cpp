...
bool ClipTokenizer::load(const std::string &vocab_json, const std::string &merges_txt) {
	if (!parse_vocab_json(vocab_json, vocab)) {
		return false;
	}
...
static void bpe_word(const ClipTokenizer &t, const std::string &word, std::vector<int32_t> &out) {
	...
	for (unsigned char c : word) {
		sym.push_back(*byte_to_unicode(c));
	}
	if (sym.empty()) {
		return;
	}
	...
	while (sym.size() >= 2) {
		...
		for (size_t i = 0; i + 1 < sym.size(); i++) {
			...
			if (it != t.merge_rank.end() && (best < 0 || it->second < best)) {
				best = it->second;
				bi = i;
			}
		}
	...
	for (const std::string &s : sym) {
		...
		if (it != t.vocab.end()) {
			...
			fprintf(stderr, "clip tokenizer: no vocab entry for '%s'\n", s.c_str());
		}
	}
}
	...
	while (i < n) {
		...
		if (c == '\'') {
			...
			for (const char *p : contr) {
				size_t l = strlen(p);
				if (lo.compare(i, l, p) == 0 && l > len) {
					len = l;
				}
			}
			if (len) {
				i += len;
				bpe_word(*this, lo.substr(beg, len), ids);
				continue;
			}
		}
		if (is_letter(c)) {
			while (i < n && is_letter((unsigned char)lo[i])) {
				i++;
			}
		} else if (isdigit(c)) {
			i++;
			...
			while (i < n) {
				...
				if (isspace(d) || is_letter(d) || isdigit(d)) {
					break;
				}
				i++;
			}
...
void clip_text_graph(Model &m, ggml_tensor *ids, const ClipParams &p,
	...
	if (final_out) {
		*final_out = layer_norm_affine(m, x, "text_model.final_layer_norm");
	}
}
