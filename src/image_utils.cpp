...
Image smart_resize(const Image &src, int tw, int th) {
	...
	if ((int64_t)tw * th < (int64_t)src.w * src.h) {
		return resize_area(src, tw, th);
	}
...
Image center_square_pad_resize(const Image &src, int target, float pad_value,
	...
	if (sq.w != target) {
		sq = smart_resize(sq, target, target);
	}
	return sq;
}
...
Image pad_rgb(const Image &rgba) {
	...
	Level cur;
	cur.pc.w = cur.a.w = rgba.w;
	cur.pc.h = cur.a.h = rgba.h;
	cur.pc.c = 3;
	cur.a.c = 1;
	...
	for (size_t i = 0; i < cur.a.data.size(); i++) {
		...
		cur.a.data[i] = a;
		for (int c = 0; c < 3; c++) {
			cur.pc.data[i * 3 + c] = rgba.data[i * 4 + c] * a;
		}
	}
	for (;;) {
		pyr.push_back(cur);
