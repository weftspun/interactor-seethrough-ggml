...
static std::string build_psd_json(const SeeThroughResult &result) {
	...
	for (size_t i = 0; i < result.png_layers.size(); i++) {
		...
		j += "\"" + psd_json_escape(tag) + "\": {\"tag\": \"" + psd_json_escape(tag) + "\", \"xyxy\": [" + std::to_string(bb[0]) + ", " + std::to_string(bb[1]) + ", " + std::to_string(bb[2]) + ", " + std::to_string(bb[3]) + "], \"depth_median\": " + std::to_string(result.layer_depth_median[i]) + "}";
	}
...
int main(int argc, char **argv) {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (a == "-m") {
			...
			png_dir = next();
		} else if (a == "--no-split-depth") {
			cfg.partseg_flags &= ~PARTSEG_DEPTH;
		} else if (a == "--no-split-lr") {
			cfg.partseg_flags &= ~PARTSEG_LR;
		} else if (a == "--split-depth-tags") {
			...
			cfg.rowchunk_budget_mb = std::stoi(next());
		} else if (a == "--linear-fast") {
			cfg.linear_fast = true;
		} else if (a == "--no-linear-fast-body") {
			cfg.linear_fast_body = false;
		} else if (a == "--no-tiled-attn") {
			cfg.tiled_attn = false;
		} else if (a == "--tiled-attn-layerdiff") {
			cfg.tiled_attn_layerdiff = true;
		} else if (a == "--decode-batch") {
			cfg.decode_batch = std::stoi(next());
		} else {
	...
	if (!run_see_through(cfg, input, result)) {
		return 1;
	}
