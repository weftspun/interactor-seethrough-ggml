...
ggml_tensor *attn_tokens(Model &m, ggml_tensor *q_src, ggml_tensor *kv_src,
	...
	if (m.tiled_naive_attn) {
		...
		for (int64_t t0 = 0; t0 < Tq; t0 += chunk) {
			...
			ggml_tensor *qc = ggml_cont(ctx, ggml_view_4d(ctx, q, hd, t1 - t0, n_head, B, q->nb[1], q->nb[2], q->nb[3], t0 * q->nb[1]));
...
ggml_tensor *geglu_ff(Model &m, ggml_tensor *x, const std::string &pre) {
	...
	for (int64_t t0 = 0; t0 < T; t0 += chunk) {
		...
		ggml_tensor *xc = ggml_cont(ctx, ggml_view_3d(ctx, x, C, t1 - t0, B, x->nb[1], x->nb[2], t0 * x->nb[1]));
...
ggml_tensor *transformer3d(Model &m, ggml_tensor *x, ggml_tensor *ehs,
	...
	const int gg = m.gn_groups;
	const float ge = m.gn_eps; // UNet resnets use
	m.gn_groups = 32;
	m.gn_eps = 1e-6f; // 32/1e-6 here
