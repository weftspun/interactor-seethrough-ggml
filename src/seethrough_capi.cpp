...
std::vector<float> conv_run(Fixture &fx, const st_case &c, bool direct, bool rowchunk,
	...
	auto r = run1(fx, [&]() {
            ...
            ggml_set_input(xt);
            return conv2d(fx.m, xt, "w", c.stride, 1); }, [&]() { ggml_backend_tensor_set(xt, x.data(), 0, x.size() * 4); });
...
std::vector<float> linear_run(Fixture &fx, const st_case &c, bool quantized,
	...
	auto r = run1(fx, [&]() {
            ...
            ggml_set_input(xt);
            return linear(fx.m, xt, "w"); }, [&]() { ggml_backend_tensor_set(xt, x.data(), 0, x.size() * 4); });
	return r;
}
...
ST_API int st_attn_probe(uint32_t heads, uint32_t tq, uint32_t tk, uint32_t batch,
	...
	st_case c = {};
	c.op = "attn";
	c.heads = (int32_t)heads;
	c.tq = (int32_t)tq;
	c.tk = (int32_t)tk;
	c.batch = (int32_t)batch;
	c.seed = seed;
	Fixture fx(seed);
	...
	if (ref.size() != cand.size() || ref.empty()) {
		return -1;
	}
...
double st_witness_check(const st_case *c) {
	...
	if (strcmp(c->op, "attn") == 0) {
		if (c->heads < 1 || c->tq < 1 || c->tk < 1 || c->batch < 1) {
			return -1.0;
		}
		Fixture fx(c->seed);
		const int C = 64 * c->heads;
	...
	if (strcmp(c->op, "linear_quant") == 0) {
		...
		if (c->c < 32 || c->c % 32 != 0 || c->oc < 1 || c->tq < 1) {
			return -1.0;
		}
		Fixture fx(c->seed);
...
struct GemmHelper {
	...
	double scale;
	uint64_t seed;
	GemmHelper(Fixture &f, int m, int n, int k, double s, uint64_t sd) : fx(f), M(m), N(n), K(k), scale(s), seed(sd) {}
