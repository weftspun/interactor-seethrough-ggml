...
int connected_components(const std::vector<uint8_t> &mask, int w, int h,
	...
	auto find = [&](int a) { while (parent[a] != a){ a = parent[a] = parent[parent[a]];
} return a; };
	...
	if (next > 0) {
		stats[0] = { 0, 0, w, h, stats[0].area }; // background full frame
	}
	return next;
}
...
void dilate_ellipse(std::vector<uint8_t> &mask, int w, int h, int k) {
	...
	for (int y = 0; y < d; y++) {
		...
		for (int x = std::max(0, x0); x <= std::min(d - 1, x1); x++) {
			ker[(size_t)y * d + x] = 1;
		}
	}
...
void gaussian3_blur(std::vector<float> &buf, int w, int h) {
	...
	auto clampi = [](int v, int lo, int hi) { return v < lo ? lo : v > hi ? hi
																		  : v; };
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			double a = 0;
			for (int t = -1; t <= 1; t++) {
				a += kk[t + 1] * buf[(size_t)y * w + clampi(x + t, 0, w - 1)];
			}
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			double a = 0;
			for (int t = -1; t <= 1; t++) {
				a += kk[t + 1] * tmp[(size_t)clampi(y + t, 0, h - 1) * w + x];
			}
...
void crop_part(Part &p) {
	...
	int64_t max_area = 0;
	for (int l = 1; l < n; l++) {
		max_area = std::max<int64_t>(max_area, stats[l].area);
	}
	...
	for (int l = 1; l < n; l++) {
		keep[l] = stats[l].area >= min_area ? 1 : 0;
	}
	...
	if (max_area > 0) {
		...
		for (int l = 1; l < n; l++) {
			...
			x0 = std::min(x0, s.x);
			y0 = std::min(y0, s.y);
			x1 = std::max(x1, s.x + s.w - 1);
			y1 = std::max(y1, s.y + s.h - 1);
		}
		for (int y = y0; y <= y1; y++) {
			for (int x = x0; x <= x1; x++) {
	...
	p.xyxy[2] = p.xyxy[0] + x1;
	p.xyxy[3] = p.xyxy[1] + y1;
	p.xyxy[0] += x0;
	p.xyxy[1] += y0;
	p.img = std::move(img2);
...
static Part process_cuts(const Part &src, const CCStats &bb,
	...
	out.img.h = ty2 - ty1;
	out.img.c = 4;
	out.depth.w = out.img.w;
	out.depth.h = out.img.h;
	out.depth.c = 1;
	...
	if (comp_mask) {
		...
		for (size_t i = 0; i < m.size(); i++) {
			...
			if (m[i]) {
				dvals.push_back(out.depth.data[i]);
			}
		}
		out.depth_median = median_of(std::move(dvals));
	}
	out.xyxy[0] = tx1 + src.xyxy[0];
	out.xyxy[1] = ty1 + src.xyxy[1];
	out.xyxy[2] = tx2 + src.xyxy[0];
	out.xyxy[3] = ty2 + src.xyxy[1];
	return out;
}
...
std::vector<Part> cluster_inpaint_part(const Part &part, const InpaintFn &inpaint) {
	...
	for (size_t i = 0; i < mask.size(); i++) {
		if (mask[i]) {
			dmin = std::min(dmin, part.depth.data[i]);
			dmax = std::max(dmax, part.depth.data[i]);
		}
	}
	...
	for (size_t i = 0; i < alpha.size(); i++) {
		...
		for (int c = 0; c < 3; c++) {
			rgb.data[i * 3 + c] = part.img.data[i * 4 + c];
		}
	}
	...
	for (size_t i = 0; i < imask.size(); i++) {
		for (int c = 0; c < 3; c++) {
			front.img.data[i * 4 + c] = rgb.data[i * 3 + c];
		}
		front.img.data[i * 4 + 3] = imask[i] / 255.0f;
		if (to_mask[i]) {
			dmed_front.push_back(part.depth.data[i]);
		}
	}
	...
	for (size_t i = 0; i < alpha.size(); i++) {
		double dist = 0;
		for (int c = 0; c < 3; c++) {
			dist += fabs(rgb.data[i * 3 + c] - fill) * 255.0 / 3.0;
		}
		double a = dist > 15.0 ? 255.0 : dist;
		if (imask_inpaint[i] <= 127) {
			a = alpha[i] * 255.0;
		}
	...
	for (size_t i = 0; i < mask.size(); i++) {
		if (valid[i]) {
			dvalid.push_back(dq[i] / 255.0f);
		}
	}
	double dmed_valid = median_of(std::move(dvalid));
	for (size_t i = 0; i < mask.size(); i++) {
		for (int c = 0; c < 3; c++) {
			back.img.data[i * 4 + c] = rgb.data[i * 3 + c];
		}
		...
		if (mask[i] && dq[i] / 255.0f >= thr) {
			dback.push_back(part.depth.data[i]);
		}
	}
