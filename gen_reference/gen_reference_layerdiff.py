...
def main():
    ...
    def rec_randn(shape, generator=None, device=None, dtype=None):
        t = orig_randn(
            shape, generator=rng, device=torch.device("cpu"), dtype=torch.float32
        )
    ...
    def rec_step(self, model_output, *a, **k):
        ...
        step_lat.append(
            (r[0] if isinstance(r, tuple) else r.prev_sample).detach().clone()
        )
        return r
    ...
    with torch.no_grad():
        c_concat_in = (
            pipe.vae.encode(rgb01_pre * 2 - 1).latent_dist.mean
    ...
    arrays = (
        [vae_feed, c_concat_in, embeds, pooled]
        + recorded
        + [latents.squeeze(0)]
        + [t.squeeze(0) for t in step_eps]
        + [t.squeeze(0) for t in step_lat]
    )
